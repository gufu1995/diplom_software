...
rpn.compile( optimizer = opt, loss = { "scores" : loss_cls, "deltas" : smoothL1 } )

#### Data Section
import glob
import pandas as pd

imgs_path = os.path.join( main_path, "prep/png_err/" )

image_list = glob.glob( imgs_path + "*.png" )
df = pd.read_csv( os.path.join( main_path, "prep/bboxes_s5.csv" ), sep = ";", decimal = "," )
...
output = create_batch( train_list[ 0 ][ 0 ], train_list[ 0 ][ 1 ], cnn = model)
...
date = f"{now.day}.{now.month}.{now.year}_{now.hour}.{now.minute:02d}"
model_name = "rpn_weights_" + date + ".hdf5"
model_name = os.path.join( train_path, "models", model_name )
mc_cb = ModelCheckpoint( filepath = os.path.join( train_path, "checkpoints", f"checkpoint_{date}.hdf5" ), verbose = 1, save_best_only = True, monitor = "loss" )
tb_cb = TensorBoard( log_dir = os.path.join( train_path, "log" ), profile_batch = 100000000 )
history = rpn.fit_generator( input_gen( ), steps_per_epoch = 100, epochs = 50, callbacks = [ mc_cb, tb_cb ] )

rpn.save( model_name )

loss = history.history[ "loss" ]

scores_loss = history.history[ "scores_loss" ]
deltas_loss = history.history[ "deltas_loss" ]
min_deltas = np.min( deltas_loss )
min_scores = np.min( scores_loss )
i_deltas = np.where( deltas_loss == min_deltas )[ 0 ][ 0 ]
i_scores = np.where( scores_loss == min_scores )[ 0 ][ 0 ]

pickle.dump( history.history, open( os.path.join( train_path, "models", "rpn_weights_" + date + ".p" ), "wb" ) )

fig, ( axest, axesb ) = plt.subplots( 2, 1, figsize = ( 8, 10 ), sharex = True, dpi = 100 )
axesb.set_xlim( [ 0, len( loss ) - 1 ] )
axesb.set_ylim( [ 0, 1000 ] )
axest.set_ylim( [ 0, 3 ] )
axest.set_title( f"Losses for Model - {date} | Trained for {len( loss )} epochs " )
axest.plot( scores_loss, label = "scores" )
axesb.plot( deltas_loss, label = "deltas" )
axesb.grid( )
axest.grid( )
axesb.set_xlabel( "Number of Epochs" )
axesb.set_ylabel( "Loss - Deltas")
axest.set_ylabel( "loss - Scores")
axest.annotate( f"loss = {min_scores:.3f} | ep = {i_scores}", xy = [ i_scores - len( loss ) / 4, 1 ])
axesb.annotate( f"loss = {int(min_deltas)} | ep = {i_deltas}", xy = [ i_deltas - len( loss ) / 4, 900 ])

fig.tight_layout( )

plt.savefig( os.path.join( train_path, "diagrams", date + ".png" ), bbox_inches = 'tight', pad_inches = 0.1 )
